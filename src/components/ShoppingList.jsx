...
const ShoppingList = ({ currentFamily }) => {
    ...
    return (
        <div className="main-content">
            ...
            <div className="shopping-card-container" style={{ background: 'white', padding: '24px', borderRadius: '16px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', maxWidth: '800px', marginBottom: 40 }}>
                
                {/* --- AÑADIR MANUAL --- */}
                <div className="shopping-form-row" style={{ position: 'relative', display: 'flex', alignItems: 'flex-start', gap: '10px', marginBottom: '30px', flexWrap: 'wrap', borderBottom: '2px solid #F3F4F6', paddingBottom: '24px' }}>
                    ...
                    <div style={{ flex: 2, minWidth: '200px', position: 'relative' }}>
                        <input 
                            ...
                            style={{ width: '100%', border: selectedIngredient ? '2px solid #22C55E' : '2px solid #E5E7EB', borderRadius: '8px', padding: '10px 14px', fontSize: '1rem', outline: 'none', background: selectedIngredient ? '#F0FFF4' : 'white', boxSizing: 'border-box' }}
                        />
                        {selectedIngredient && (
                        ...
                        {showSuggestions && suggestions.length > 0 && (
                            <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 999, background: 'white', border: '2px solid #E5E7EB', borderRadius: 8, boxShadow: '0 8px 24px rgba(0,0,0,0.12)', maxHeight: 200, overflowY: 'auto', marginTop: 4 }}>
                                {suggestions.map(ing => (
                        ...
                        {creatingNew && (
                            <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 999, background: '#FFFBF5', border: '2px dashed #FFD9A0', borderRadius: 8, padding: 12, marginTop: 4, display: 'flex', gap: 8 }}>
                                <select value={newIngredient.unit} onChange={e => setNewIngredient({...newIngredient, unit: e.target.value})} style={{ flex: 1, padding: '6px', borderRadius: 6, border: '1px solid #E5E7EB' }}>
                    ...
                    <input 
                        ...
                        style={{ width: '80px', border: '2px solid #E5E7EB', borderRadius: '8px', padding: '10px', fontSize: '1rem', outline: 'none' }}
                    />
                    
                    {!selectedIngredient && (
                        <input type="text" placeholder="Unidad" value={unit} onChange={e => setUnit(e.target.value)} style={{ width: '80px', border: '2px solid #E5E7EB', borderRadius: '8px', padding: '10px', fontSize: '1rem', outline: 'none' }} />
                    )}

                    <button onClick={handleAddManual} style={{ background: '#F3F4F6', color: '#374151', border: 'none', padding: '10px 20px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', flexShrink: 0 }}>
                        Añadir
                    </button>
                </div>

                {/* --- LISTA POR COMPRAR --- */}
                <h3 style={{ fontSize: '1.1rem', color: '#374151', marginBottom: 16, display: 'flex', justifyContent: 'space-between' }}>
                    Por Comprar
                    <span style={{ background: '#FEE2E2', color: '#DC2626', padding: '2px 10px', borderRadius: 20, fontSize: '0.8rem', fontWeight: 700 }}>{toBuy.length}</span>
                </h3>
                <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 30px 0' }}>
                    {loading ? <p style={{ color: '#9CA3AF' }}>Cargando...</p> : toBuy.length === 0 ? <p style={{ color: '#9CA3AF', fontSize: '0.9rem' }}>Todo al día.</p> : toBuy.map(item => (
                        <li key={item.item_id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 12px', borderBottom: '1px solid #F9FAFB', transition: 'background 0.2s', borderRadius: 8 }} onMouseEnter={e => e.currentTarget.style.background = '#F9FAFB'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', cursor: 'pointer', flex: 1 }} onClick={() => openInventoryModal(item)}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column' }}>
                                    <span style={{ fontSize: '1.05rem', color: '#374151', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}>
                                        {item.name}
                                    ...
                                    <span style={{ fontSize: '0.85rem', color: '#6B7280', fontWeight: '500' }}>{item.quantity} {item.unit}</span>
                                </div>
                            </div>
                ...
                {bought.length > 0 && (
                    <>
                        <h3 style={{ fontSize: '1.1rem', color: '#9CA3AF', marginBottom: 16, display: 'flex', justifyContent: 'space-between', borderTop: '2px dashed #F3F4F6', paddingTop: 24 }}>
                            Ya en el carrito (Inventario)
                            <span style={{ background: '#DCFCE7', color: '#16A34A', padding: '2px 10px', borderRadius: 20, fontSize: '0.8rem', fontWeight: 700 }}>{bought.length}</span>
                        </h3>
                        <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 20px 0', opacity: 0.7 }}>
                            {bought.map(item => (
                                <li key={item.item_id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px', borderBottom: '1px solid #F9FAFB' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px', cursor: 'pointer', flex: 1 }} onClick={() => openInventoryModal(item)}>
                ...
                <div className="modal-overlay" onClick={() => setShowModal(false)}>
                    <div className="modal-modern" onClick={e => e.stopPropagation()} style={{ maxWidth: 400 }}>
                        <div style={{ padding: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #F3F4F6' }}>
                            <h3 style={{ margin: 0, fontWeight: 800, color: '#374151' }}>Confirmar Ingreso</h3>
                            <button onClick={() => setShowModal(false)} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
                                <X size={22} color="#9CA3AF" />
                            </button>
                        </div>
                        <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                            <div style={{ background: '#F3F4F6', padding: '16px', borderRadius: 12, textAlign: 'center' }}>
                                <h2 style={{ margin: 0, color: '#1F2937' }}>{modalItem.name}</h2>
                                <p style={{ margin: '4px 0 0', color: '#6B7280', fontSize: '0.9rem' }}>Se guardará en la nevera</p>
                            </div>

                            <div style={{ display: 'flex', gap: 12 }}>
                                <div style={{ flex: 1 }}>
                                    <label style={{ fontSize: '0.85rem', fontWeight: 600, color: '#4B5563', marginBottom: 6, display: 'block' }}>Cantidad comprada</label>
                                    <input type="number" min="0" step="any" value={modalQty} onChange={e => setModalQty(e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: 8, border: '2px solid #E5E7EB', boxSizing: 'border-box' }} />
                                </div>
                                <div style={{ width: 80 }}>
                                    <label style={{ fontSize: '0.85rem', fontWeight: 600, color: '#4B5563', marginBottom: 6, display: 'block' }}>Unidad</label>
                                    <input type="text" value={modalItem.unit} disabled style={{ width: '100%', padding: '10px', borderRadius: 8, border: '2px solid #E5E7EB', background: '#F9FAFB', color: '#9CA3AF', boxSizing: 'border-box' }} />
                                </div>
                            </div>

                            <div>
                                <label style={{ fontSize: '0.85rem', fontWeight: 600, color: '#4B5563', marginBottom: 6, display: 'block' }}>Fecha de vencimiento (Opcional)</label>
                                <input type="date" value={modalExpDate} onChange={e => setModalExpDate(e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: 8, border: '2px solid #E5E7EB', boxSizing: 'border-box' }} />
                            </div>
